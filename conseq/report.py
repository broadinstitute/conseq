...
def generate_report_cmd(state_dir, dest_dir):
    ...
    def value_cell(value):
        if isinstance(value, dict):
            if "$value" in value:
                return Markup(
                    f"<td class='transient-value'>{escape(value['$value'])}</td>"
                )
            elif "$filename" in value:
                return Markup(
                    f"<td class='filename-value'>{escape(value['$filename'])}</td>"
                )
            elif "$file_url" in value:
                return Markup(
                    f"<td class='file-url-value'>{escape(value['$file_url'])}</td>"
                )
        return Markup(f"<td>{escape(str(value))}</td>")
