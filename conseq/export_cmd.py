    ...
    for props in objs:
        for k, v in props.items():
            if isinstance(v, dict) and "$filename" in v:
                ...
                assert not os.path.isdir(filename), "Cannot export artifacts which reference a directory: {}".format(
                    filename)
    ...
    with j.transaction():
        for rule_execution in j.rule_set:
            ...
            if execution_result is None:
                print(
                    "Skipping {} because while we had an execution id {} but appeared to be missing from the db".format(
                        rule_execution, rule_execution.execution_id))
                continue
