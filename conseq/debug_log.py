...
def log_execute(name, id, job_dir, inputs, run_stmts):
    _log(dict(type="execute", name=name, id=id, job_dir=job_dir, inputs=[repr(i) for i in inputs], run_stmts=run_stmts,
              run_stmts_hash=_hash(repr(run_stmts))))


def log_input_preprocess(id, orig_inputs, processed_inputs):
    _log(dict(type="preprocess", id=id, orig_inputs=[repr(i) for i in orig_inputs], processed_inputs=processed_inputs))
