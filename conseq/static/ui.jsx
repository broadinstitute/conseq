...
var PropConstButton = React.createClass({
  ...
  render: function() {
    return (
    <div className="row">
      <div className="altlabel">{ this.props.label } <span className="glyphicon glyphicon-remove altbadge" onClick={this.removeClicked}></span>
      </div>
    </div>
  )
  }
...
var CommonBlock = React.createClass({
  render: function() {
      ...
      var header = this.props.properties.map(function(x, i) { return (
          <th key={"h"+i}>{ x } </th>
       )});
      ...
      for(var i=0;i<t.props.instances.length;i++)
      {
          ...
          var tds = this.props.properties.map(function(x, i) { return (
          <td key={i}>{ instance[x] } </td>
       )});
    ...
    return (
    <table className="table table-striped"><tbody><tr>{ header }</tr>{ rows }</tbody></table>
  )
  }
});
...
var QueryTool = React.createClass({
  removeFilter: function(i) {
      ...
      var newState = upd(this.state, {valueSelection:{$set: ""}, propertySelection: {$set: ""}, predicates: {$splice: [[i, 1]]}});
  ...
  addFilter: function(e) {
    var pair = {property: this.state.propertySelection, value: this.state.valueSelection}
    var newState = upd(this.state, {valueSelection:{$set: ""}, propertySelection: {$set: ""}, predicates: {$push: [pair]}});
    this.setState(newState)
    this.loadPropertyList(newState.predicates);
  },
  ...
  render: function() {
    ...
    var makeButton = function(pred, i) {
      return <PropConstButton key={i} index={i} label={pred.property + " is "+pred.value} removeCallback={t.removeFilter}/>
    }

    var makeOptions = function(o, i) {
        // why does  ({o.count}) result in undef error?
      return <option key={i} value={o.label}>{o.label}</option>
    }
    ...
    if (this.state.propertySelection != "") {
      ...
      valueSelNodes.push(<div className="form-group">
        <select value={this.state.valueSelection} className="form-control" onChange={this.selectValue}>
    ...
    <div className="form-inline">
      ...
      <select value={this.state.propertySelection} className="form-control" onChange={this.selectProperty}>
        <option value="">Select property...</option>
        {propertyOptions}
      </select>
      </div>
      {valueSelNodes}
        <div><CommonBlock instances={this.state.instances.instances} properties={this.state.instances.properties}/></div>
    </div>
    );

    var buttons = this.state.predicates.map(makeButton);
    return <div>{buttons}<div className="row">{e}</div></div>;
  }
});

var fetchProperties = function(query) {
    query = query.map(function(x) { return [x.property, x.value] });
    ...
    return $.ajax("/api/props", {
        ...
        return props.map(function(x) { return {label: x[0], count: x[1]} } )
    });
  //var result = [
  //      {label: "type", count: 10},
  //      {label: "name", count: 14},
...
var fetchValues = function(query, property) {
    ...
    return $.ajax("/api/values", {
        ...
        return values.map(function(x) { return {label: x[0], count: x[1]} } )
    });
  //var result = [
...
$(function(){
    ...
    ReactDOM.render(<QueryTool fetchProperties={fetchProperties} fetchValues={fetchValues} fetchInstances={fetchInstances} />, mountNode);
});
