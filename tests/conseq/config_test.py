...
def test_parsing_default_config_in_home_dir(tmpdir, monkeypatch):
    ...
    def _mock_export_cmd(state_dir, depfile, config_file, dest_gs_path, exclude_patterns):
    ...
    main(["--dir", str(tmpdir.join("state")), "export", str(tmpdir.join("sample.conseq")), "gs://test" ])
